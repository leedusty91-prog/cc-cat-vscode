...
### Features
- **重命名会话**:每张卡片标题旁新增 ✎ 按钮,可自定义会话名称(hover 显示)。回车保存、Esc 取消、留空恢复原始标题。自定义名称优先作为展示标题,且参与搜索。名称只存到 sidecar `categories.json`(新增 `name` 字段),**不改动原始 .jsonl**,向后兼容旧索引格式。中英双语文案已补齐。
